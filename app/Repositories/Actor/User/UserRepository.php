...
class UserRepository extends Repository
{
	...
	public function confirmAccount($token)
	{
		...
		if ($user->confirmed == 1) {
			throw new GeneralException(trans('exceptions.actor.user.auth.confirmation.already_confirmed'));
		}
		...
		throw new GeneralException(trans('exceptions.actor.user.auth.confirmation.mismatch'));
	}
		...
		if ($user->canChangeEmail()) {
			...
			if ($user->email != $input['email']) {
				...
				if ($this->findByEmail($input['email'])) {
					throw new GeneralException(trans('exceptions.actor.user.auth.email_taken'));
				}
	...
	public function changePassword($input)
	{
		...
		throw new GeneralException(trans('exceptions.actor.user.auth.password.change_mismatch'));
	}
}
