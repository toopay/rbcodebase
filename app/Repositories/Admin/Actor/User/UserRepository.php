...
class UserRepository extends Repository
{
	...
	public function getForDataTable($status = 1, $trashed = false)
	{
		...
		$dataTableQuery = $this->query()
			->with('roles')
			->select([
				config('actor.users_table') . '.id',
				config('actor.users_table') . '.name_first',
				config('actor.users_table') . '.name_last',
				config('actor.users_table') . '.name_slug',
				config('actor.users_table') . '.email',
				config('actor.users_table') . '.status',
				config('actor.users_table') . '.confirmed',
				config('actor.users_table') . '.created_at',
				config('actor.users_table') . '.updated_at',
				config('actor.users_table') . '.deleted_at',
			]);
