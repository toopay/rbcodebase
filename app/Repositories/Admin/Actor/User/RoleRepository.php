...
class RoleRepository extends Repository
{
	...
	public function getAll($order_by = 'sort', $sort = 'asc')
	{
		return $this->query()
			...
			->select([
				config('actor.roles_table') . '.id',
				config('actor.roles_table') . '.name',
				config('actor.roles_table') . '.all',
				config('actor.roles_table') . '.sort',
			]);
	}
	...
	public function create(array $input)
	{
		...
		if (! $all) {
			...
			if (config('actor.roles.role_must_contain_permission') && count($input['permissions']) == 0) {
	...
	public function getDefaultUserRole() {
		...
		return $this->query()->where('name', config('actor.users.default_role'))->first();
	}
}
