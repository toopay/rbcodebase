...
trait UseSocialite
{
	...
	public function loginThirdParty(Request $request, $provider)
	{
		...
		if (! in_array($provider, $this->getAcceptedProviders())) {
			return redirect()->route('app.index')->withFlashDanger(trans('auth.socialite.unacceptable', ['provider' => $provider]));
		}
		...
		if (! access()->user()->isActive()) {
			auth()->logout();
			throw new GeneralException(trans('exceptions.app.auth.deactivated'));
		}
	...
	protected function getSocialLinks()
	{
		...
		if (strlen(getenv('BITBUCKET_CLIENT_ID'))) {
			$socialite_enable[] = link_to_route('actor.user.auth.provider', trans('labels.app.auth.login_with', ['social_media' => 'Bit Bucket']), 'bitbucket');
		}

		if (strlen(getenv('FACEBOOK_CLIENT_ID'))) {
			$socialite_enable[] = link_to_route('actor.user.auth.provider', trans('labels.app.auth.login_with', ['social_media' => 'Facebook']), 'facebook');
		}

		if (strlen(getenv('GOOGLE_CLIENT_ID'))) {
			$socialite_enable[] = link_to_route('actor.user.auth.provider', trans('labels.app.auth.login_with', ['social_media' => 'Google']), 'google');
		}

		if (strlen(getenv('GITHUB_CLIENT_ID'))) {
			$socialite_enable[] = link_to_route('actor.user.auth.provider', trans('labels.app.auth.login_with', ['social_media' => 'Github']), 'github');
		}

		if (strlen(getenv('LINKEDIN_CLIENT_ID'))) {
			$socialite_enable[] = link_to_route('actor.user.auth.provider', trans('labels.app.auth.login_with', ['social_media' => 'Linked In']), 'linkedin');
		}

		if (strlen(getenv('TWITTER_CLIENT_ID'))) {
			$socialite_enable[] = link_to_route('actor.user.auth.provider', trans('labels.app.auth.login_with', ['social_media' => 'Twitter']), 'twitter');
		}
